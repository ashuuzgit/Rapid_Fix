...
function MorphingPopoverTrigger({
  ...
  if (asChild && isValidElement(children)) {
    // Cast children.type to any since framer-motion create works slightly differently on exact intrinsic element types in strict TS 
      ...
      {context.isOpen && (
        <>
          <motion.div
            ...
            aria-modal='true'
            className={cn(
              'absolute overflow-hidden rounded-xl border border-white/10 bg-[#1A1A1A] p-6 text-white shadow-xl flex flex-col items-center justify-center -translate-y-24 z-50',
              className
            )}
            initial='initial'
            animate='animate'
